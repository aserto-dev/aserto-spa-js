...
export default class AsertoClient {
  ...
  async reload(body: RequestInit['body'], headers: Record<string, string> = {}) {
    const result = await post(this.service, this.token, this.endpoint, body, headers)

    if (result.error !== null) {
      const { error } = result
      throw new Error(`AsertoClient: ${error instanceof Error ? error.message : error}`)
    } else {
  ...
  getDisplayState(method: string, path?: string, policyRoot?: string): Record<string, boolean> {
